...
interface StorefrontWorkspaceProps {
    initialSection?: StorefrontSection;
}

export default function StorefrontWorkspace({
    initialSection = DEFAULT_STOREFRONT_SECTION,
}: StorefrontWorkspaceProps) {
    ...
    const canCTACustomize = canUsePlanFeature(companyUser?.company, "HOME_CTA_CUSTOMIZATION");
    const canSectionOrder = canUsePlanFeature(companyUser?.company, "HOME_SECTION_ORDER");
    const canFooterCustomize = canUsePlanFeature(companyUser?.company, "FOOTER_CUSTOMIZATION");
    const canBanners = canUsePlanFeature(companyUser?.company, "ANNOUNCEMENT_BANNERS");
    const homeCtaRequiredPlan = getRequiredPlanForFeature(companyUser?.company, "HOME_CTA_CUSTOMIZATION");
    const homeSectionOrderRequiredPlan = getRequiredPlanForFeature(companyUser?.company, "HOME_SECTION_ORDER");
    const footerRequiredPlan = getRequiredPlanForFeature(companyUser?.company, "FOOTER_CUSTOMIZATION");
    const bannersRequiredPlan = getRequiredPlanForFeature(companyUser?.company, "ANNOUNCEMENT_BANNERS");
    ...
    const storefrontHref = useMemo(() => {
        ...
        return slug ? `/shop/${slug}` : "/admin/dashboard/business-settings";
    ...
    const fetchBuilderData = useCallback(async () => {
        ...
        try {
            const [themeRes, contentRes, staffRes, companyRes, settingsRes] = await Promise.all([
                fetch(resolveApiUrl("/admin/theme"), {
                    headers: { "Content-Type": "application/json" },
                    credentials: "include",
                }),
                fetch(resolveApiUrl(`/api/admin/company/${companyId}/content`), {
                    credentials: "include",
                }),
                fetch(resolveApiUrl(`/api/admin/staff?company_id=${companyId}`), {
                    credentials: "include",
                }),
            ...
            if (company.slug) {
                const publicRes = await fetch(resolveApiUrl(`/api/company/${company.slug}`), {
                    credentials: "include",
                });
                if (publicRes.ok) {
    ...
    const handleImageDelete = async (field: keyof CompanyContent) => {
        ...
        try {
            const response = await fetch(resolveApiUrl(`/api/admin/company/${companyId}/content`), {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                credentials: "include",
    ...
    const saveTheme = async () => {
        ...
        const response = await fetch(resolveApiUrl("/admin/theme"), {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
    ...
    const saveContent = async () => {
        ...
        for (const [key, file] of Object.entries(pendingImages)) {
            ...
            const response = await fetch(resolveApiUrl("/api/admin/uploads/image"), {
                method: "POST",
                body: formData,
                credentials: "include",
            });
        ...
        const contentResponse = await fetch(resolveApiUrl(`/api/admin/company/${companyId}/content`), {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
            body: JSON.stringify({
    ...
    const handleSectionChange = (value: string) => {
        const nextSection = value as StorefrontSection;
        setActiveSection(nextSection);
        router.push(`/admin/dashboard/storefront/${nextSection}`);
    };
    ...
    return (
        <AdminPageShell className="max-w-[1600px] pb-24 md:pb-0">
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]">
                <Tabs value={activeSection} onValueChange={handleSectionChange} className="min-w-0 space-y-4">
                            ...
                            <CardContent>
                                {!canCTACustomize ? (
                                    <PlanUpgradeNotice
                                        ...
                                        currentPlan={plan}
                                        requiredPlan={homeCtaRequiredPlan}
                                    />
                                ) : (
                                    <div className="space-y-3">
                                        <OutcomeHint>{t("adminTheme.ctaOutcomeHint")}</OutcomeHint>
                                        {themeConfig.homeCTAButtons.map((button, index) => {
                                            const needsEventsWarn = (button.destination === "events" || button.destination === "free-events") && !canUsePlanFeature(companyUser?.company, "GROUP_EVENTS");
                                            const needsClassesWarn = button.destination === "classes" && !canUsePlanFeature(companyUser?.company, "GROUP_CLASSES");
                            ...
                            <CardContent>
                                {!canSectionOrder ? (
                                    <PlanUpgradeNotice
                                        ...
                                        currentPlan={plan}
                                        requiredPlan={homeSectionOrderRequiredPlan}
                                    />
                                ) : (
                            ...
                            <CardContent>
                                {!canFooterCustomize ? (
                                    <PlanUpgradeNotice
                                        ...
                                        currentPlan={plan}
                                        requiredPlan={footerRequiredPlan}
                                    />
                                ) : (
                            ...
                            <CardContent>
                                {!canBanners ? (
                                    <PlanUpgradeNotice
                                        ...
                                        currentPlan={plan}
                                        requiredPlan={bannersRequiredPlan}
                                    />
                                ) : (
                    ...
                    <TabsContent value="business" className="space-y-4">
                        <Card className="border-surface-border bg-surface shadow-card">
                            ...
                            <CardContent className="grid gap-5 sm:grid-cols-2">
                                ...
                                <div className="grid gap-2 sm:grid-cols-4">
                                    <Field label={t("adminSettings.prefix")} id="phone_prefix" className="sm:col-span-1">
                                        <Input
                                            id="phone_prefix"
                                    ...
                                    <Field label={t("adminSettings.phone")} id="phone" className="sm:col-span-3">
                        ...
                        <Card className="border-surface-border bg-surface shadow-card">
                            <CardHeader>
                                <CardTitle>{t("adminSettings.tosTab")}</CardTitle>
                                <CardDescription>{t("adminSettings.tosDesc")}</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-3">
                                <TextAreaField
                                    id="custom-tos"
                                    label={t("adminSettings.tosTitle")}
                                    value={settings.custom_tos}
                                    onChange={(value) => updateSettings("custom_tos", value)}
                ...
                <aside className="space-y-4 xl:sticky xl:top-20 xl:self-start">
                    ...
                    <Card className="border-surface-border bg-surface shadow-card">
                        <CardHeader>
                            <CardTitle className="text-base">{t("adminNav.groups.storefront")}</CardTitle>
                            <CardDescription>{t("storefrontBuilder.previewManagedContent")}</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-3 text-sm text-text-muted">
                            <p>{t("storefrontBuilder.advancedSettingsHint")}</p>
                            <div className="flex flex-wrap gap-2 pt-1">
                                <Link href="/admin/dashboard/business-settings">
