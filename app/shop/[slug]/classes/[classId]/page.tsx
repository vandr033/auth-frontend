    ...
    <main className="min-h-screen bg-page text-text-main">
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-4 py-8 md:grid-cols-[1.2fr_0.8fr] md:px-8">
        ...
        <aside className="space-y-4">
          ...
          {company.phone?.trim() ? (
            <article className="space-y-3 rounded-xl border border-surface-border bg-surface p-4 shadow-card">
              <p className="text-sm font-medium text-text-main">
                {t("shopGroup.actions.contactBusiness", { name: company.name })}
              </p>
              <a
                href={`https://wa.me/${(company.phone_prefix ?? "").replace(/\D/g, "")}${company.phone.replace(/\D/g, "")}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex w-full items-center justify-center gap-2 rounded-md border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-sm font-semibold text-emerald-700 transition-colors hover:bg-emerald-100"
              >
