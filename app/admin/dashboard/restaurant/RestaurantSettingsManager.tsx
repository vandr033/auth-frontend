...
export function RestaurantSettingsManager() {
  ...
  const addPeriod = async () => { try { const created = await createRestaurantServicePeriod({ ...draftPeriod, name: draftPeriod.name || null }); setPeriods((current) => [...current, created.period]); if (created.message?.includes("fuera del horario general")) await notify.warning(created.message); setDraftPeriod(DEFAULT_PERIOD); } catch (cause) { await notify.error(message(cause)); } };
  const mutatePeriod = async (period: RestaurantServicePeriod, changes: Partial<RestaurantServicePeriod>) => { try { const updated = await updateRestaurantServicePeriod(period.id, changes); setPeriods((current) => current.map((item) => item.id === updated.period.id ? updated.period : item)); if (updated.message?.includes("fuera del horario general")) await notify.warning(updated.message); } catch (cause) { await notify.error(message(cause)); } };
  ...
  return <div className="mx-auto max-w-5xl space-y-8 pb-10">
    {access?.enabled && <p className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-900">{t("restaurant.servicePeriodsPolicy")}</p>}
