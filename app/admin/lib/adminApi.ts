...
export const getRestaurantWaitlistRecommendations = (params: { table_id?: number; at?: string } = {}) => { const query = new URLSearchParams(Object.entries(params).filter(([, value]) => value !== undefined).map(([key, value]) => [key, String(value)])); return restaurantRequest<Array<{ waitlist_id: number; recommended_table: RestaurantTable | null; reasons: string[]; score: number }>>(`/waitlist/recommendations${query.size ? `?${query}` : ""}`); };
...
export type RestaurantFinancialMetrics = { totals: { visits: number; guests: number; subtotal_amount_cents: number; discount_amount_cents: number; tip_amount_cents: number; total_paid_amount_cents: number; average_spend_cents: number; average_guest_spend_cents: number }; by_payment_method: Array<Record<string, unknown>>; by_waiter: Array<Record<string, unknown>>; by_area: Array<Record<string, unknown>>; by_shift: Array<Record<string, unknown>>; };
